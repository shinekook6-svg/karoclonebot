...
spin_items = load_json(SPIN_FILE, [])       # list of {"name":"🎉 Gold", "chance":20}
...
def main_reply_keyboard():
    ...
    kb.row(KeyboardButton("🎡 Spin"), KeyboardButton("👥 Invite"))
    kb.row(KeyboardButton("💱 Exchange"), KeyboardButton("💰 Check Balance"))
    kb.row(KeyboardButton("🛠 Admin Panel"))
    return kb

def inline_yes_no(redeem_id):
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("✅ Redeem", callback_data=f"redeem_do:{redeem_id}"),
           InlineKeyboardButton("✖ Cancel", callback_data=f"redeem_cancel:{redeem_id}"))
    return kb

def admin_approve_buttons(pending_id):
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("✔ Approve", callback_data=f"admin_approve:{pending_id}"),
           InlineKeyboardButton("✖ Reject", callback_data=f"admin_reject:{pending_id}"))
    return kb

# add admin panel ad-plan buttons and addpoints button
def admin_panel_keyboard():
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("➕ Add Reward", callback_data="admin_add_reward"),
           InlineKeyboardButton("📝 Edit Reward", callback_data="admin_edit_reward"))
    kb.add(InlineKeyboardButton("📋 List Rewards", callback_data="admin_list_reward"))
    kb.add(InlineKeyboardButton("🗑 Delete Reward", callback_data="admin_del_reward"))
    kb.add(InlineKeyboardButton("➕ Add Spin Item", callback_data="admin_add_spin"),
           InlineKeyboardButton("📝 Edit Spin Item", callback_data="admin_edit_spin"))
    kb.add(InlineKeyboardButton("📋 List Spin Items", callback_data="admin_list_spin"))
    kb.add(InlineKeyboardButton("🗑 Delete Spin Item", callback_data="admin_del_spin"))
    # new ad-plan buttons
    kb.add(InlineKeyboardButton("➕ Add Ad Plan", callback_data="admin_add_adplan"),
           InlineKeyboardButton("📋 List Ad Plans", callback_data="admin_list_adplan"))
    kb.add(InlineKeyboardButton("🗑 Delete Ad Plan", callback_data="admin_del_adplan"))
    # addpoints helper
    kb.add(InlineKeyboardButton("➕ Add Points", callback_data="admin_add_points"))
    kb.add(InlineKeyboardButton("📊 Users Count", callback_data="admin_users_count"))
    return kb
        ...
        if text and text.startswith("/start ref_"):
            ...
            if str(ref) != str(user_id):
                if users[str(user_id)].get("invited_by") is None:
                    ...
                    save_json(USERS_FILE, users)
                    try:
                        bot.send_message(ref, f"🎉 Someone joined with your link! +10 points added.")
                    except Exception:
                        pass
                    try:
                        bot.send_message(ADMIN_ID, f"🔔 Referral: @{users[str(ref)].get('username','')} invited @{username or first_name}")
                    except Exception:
                        pass
...
def handle_text(msg):
    try:
        ...
        if text == "🎡 Spin": return handle_spin_text(msg)
        if text == "👥 Invite": return handle_invite_text(msg)
        if text == "💱 Exchange": return handle_exchange_text(msg)
        if text == "💰 Check Balance": return handle_balance_text(msg)
        if text == "📢 Ads": return handle_ads_text(msg)
        if text == "🛠 Admin Panel": return handle_admin_panel_text(msg)
...
def handle_invite_text(msg):
    ...
    send_top_ad(msg.chat.id)
    bot.send_message(msg.chat.id, f"🔗 Your invite link:\n{link}")
...
def handle_exchange_text(msg):
    ...
    send_top_ad(msg.chat.id)
    if not rewards:
        return bot.send_message(msg.chat.id, "❌ No rewards added yet. Admin can add via /addreward")
    kb = InlineKeyboardMarkup()
    for name, cost in rewards.items():
        kb.add(InlineKeyboardButton(f"{name} - {cost} pts", callback_data=f"redeem:{name}"))
    bot.send_message(msg.chat.id, "🎁 Choose a reward:", reply_markup=kb)
...
def handle_ads_text(msg):
    ...
    if ad_plans:
        for days_str, price in ad_plans.items():
            label = f"{days_str} days — {price} MMK"
    ...
    kb.add(InlineKeyboardButton("❌ Cancel", callback_data="ads_cancel"))
    bot.send_message(msg.chat.id, "📢 Ads — choose a plan to post Top Fixed Ad:", reply_markup=kb)
...
def handle_admin_panel_text(msg):
    if msg.from_user.id not in ADMIN_IDS:
        return bot.send_message(msg.chat.id, "❌ Admin only")
    send_top_ad(msg.chat.id)
    bot.send_message(msg.chat.id, "⚙ Admin Panel", reply_markup=admin_panel_keyboard())
...
def handle_spin_text(msg):
    ...
    if u.get("free_spins", 0) > 0:
        ...
        if not item:
            return bot.send_message(msg.chat.id, "❌ No spin items yet.")
        _process_spin_result(msg.chat.id, msg.from_user, item)
        return
    pts = u.get("points", 0)
    if pts < 100:
        return bot.send_message(msg.chat.id, "❌ You need 100 points to spin.")
...
def _process_spin_result(chat_id, from_user, item):
    ...
    if parsed["type"] == "sorry":
        bot.send_message(chat_id, "😢 Result: Sorry / Try Again")
        return
    if parsed["type"] == "points":
        ...
        save_json(USERS_FILE, users)
        bot.send_message(chat_id, f"🎉 You won {amt} points!")
        return
    bot.send_message(chat_id, f"🎉 You won: {item['name']}")
    uname = from_user.username or from_user.first_name or str(from_user.id)
    for adm in ADMIN_IDS:
        try:
            bot.send_message(adm, f"🔔 User @{uname} won **{item['name']}**")
        except Exception:
            pass
...
def callback_query(call):
    try:
        ...
        if data.startswith("buyad:"):
            ...
            bot.answer_callback_query(call.id, f"Selected {days_str} days — {price} MMK. Please send your ad using /submitad {price}|{days_str}|TEXT|LINK(optional) or send a photo with caption: /submitad {price}|{days_str}|TEXT|LINK")
            bot.send_message(call.message.chat.id, f"📎 Send your ad now.\nFormat (example):\n/submitad {price}|{days_str}|My ad text here|https://t.me/mychannel\nOr send a photo with that caption.")
            return
        ...
        if data.startswith("redeem:"):
            ...
            if reward_name not in rewards:
                return bot.answer_callback_query(call.id, "❌ Reward not found")
            ...
            if pts < cost:
                return bot.answer_callback_query(call.id, f"❌ Not enough points. Need {cost}, you have {pts}.")
            ...
            save_pending()
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f"📩 Request sent for {reward_name}. Waiting admin approval.")
            buttons = admin_approve_buttons(pending_id)
            for adm in ADMIN_IDS:
                bot.send_message(adm, f"🔔 Redeem request from @{pending[pending_id]['username']}:\n🎁 {reward_name}\n💰 {cost} pts", reply_markup=buttons)
            return
        ...
        if data == "admin_add_spin":
            bot.send_message(call.message.chat.id, "Use command: /addspin NAME=CHANCE (e.g. 🎁Gold=20)")
            return
...
def _admin_approve_pending(call, pending_id):
    ...
    if pid not in pending:
        bot.answer_callback_query(call.id, "❌ Pending not found.")
        return
    entry = pending[pid]
    if entry["type"] == "reward":
        ...
        if users.get(uid, {}).get("points", 0) < cost:
            bot.send_message(call.message.chat.id, "❌ User doesn't have enough points now.")
            return
        users[uid]["points"] -= cost
        save_json(USERS_FILE, users)
        bot.send_message(call.message.chat.id, f"✅ Approved reward {entry['reward']} for user {uid}")
        try:
            bot.send_message(int(uid), f"✅ Your redeem for {entry['reward']} has been approved by admin.")
        except Exception:
            pass
        del pending[pid]
        save_pending()
    ...
    if entry["type"] == "ad":
        ...
        save_ads()
        bot.send_message(call.message.chat.id, f"✅ Ad approved and posted: {ad_id}")
        try:
            bot.send_message(int(uid), "✅ Your ad post has been approved and is live.")
        except Exception:
            pass
        del pending[pid]
        save_pending()
    ...
    bot.send_message(call.message.chat.id, "❌ Unknown pending type.")
...
def _admin_reject_pending(call, pending_id):
    ...
    try:
        if uid:
            bot.send_message(int(uid), f"❌ Your request ({entry.get('type')}) was rejected by admin.")
    except Exception:
        pass
    del pending[pid]
    save_pending()
...
def cmd_addspin(msg):
    ...
    try:
        ...
        save_spin_items()
        bot.reply_to(msg, f"✔ Added spin item: {name} ({chance}%)")
    ...
    try:
        ...
        for it in spin_items:
            if it["name"].lower() == name.lower():
                ...
                found = True
        if not found:
            return bot.reply_to(msg, "❌ Item not found")
        save_spin_items()
        bot.reply_to(msg, f"✔ Updated: {name} -> {chance}")
    except Exception as e:
        bot.reply_to(msg, f"❌ Error: {e}")
...
def cmd_delspin(msg):
    try:
        ...
        if len(spin_items) < before_count:
            bot.send_message(msg.chat.id, f"🗑 Deleted spin item: {name}")
        else:
            bot.send_message(msg.chat.id, f"❌ Spin item not found: {name}")
...
def cmd_addreward(msg):
    ...
    try:
        ...
        rewards[name] = price
        save_rewards_file()
        bot.reply_to(msg, f"✔ Added reward: {name} = {price} pts")
    except Exception as e:
        bot.reply_to(msg, f"❌ Error: {e}")
...
def cmd_editreward(msg):
    ...
    try:
        ...
        if name not in rewards:
            return bot.reply_to(msg, "❌ Reward not found")
        rewards[name] = price
        save_rewards_file()
        bot.reply_to(msg, f"✔ Updated reward: {name} = {price} pts")
    except Exception as e:
        bot.reply_to(msg, f"❌ Error: {e}")
...
def cmd_delreward(msg):
    ...
    try:
        ...
        del rewards[name]
        save_rewards_file()
        bot.reply_to(msg, f"🗑 Deleted reward: {name}")
...
def cmd_submitad(msg):
    ...
    try:
        ...
        if len(p) < 3:
            return bot.reply_to(msg, "❌ Wrong format. Use:\n/submitad PRICE|DAYS|TEXT|LINK(optional)\nOr send a photo with that caption.")
        price = int(p[0])
        days = int(p[1])
        text = p[2]
        ...
        for adm in ADMIN_IDS:
            try:
                ...
                buttons = admin_approve_buttons(pending_id)
                if photo_file_id:
                    bot.send_photo(adm, photo_file_id, caption=f"🔔 New ad request from @{pending[pending_id]['username']}\nPrice: {price}\nDays: {days}\nText: {text}\nLink: {link}", reply_markup=buttons)
                else:
                    bot.send_message(adm, f"🔔 New ad request from @{pending[pending_id]['username']}\nPrice: {price}\nDays: {days}\nText: {text}\nLink: {link}", reply_markup=buttons)
            except Exception:
                pass
        bot.reply_to(msg, "📩 Your Ad request was submitted. Waiting for admin approval.")
    except Exception as e:
        traceback.print_exc()
        bot.reply_to(msg, f"❌ Error parsing: {e}")
...
def cmd_addadplan(msg):
    ...
    try:
        ...
        ad_plans[days] = price
        save_ad_plans()
        bot.reply_to(msg, f"✔ Added ad plan: {days} days = {price} MMK")
    except Exception as e:
        bot.reply_to(msg, f"❌ Error: {e}")
...
def cmd_deladplan(msg):
    ...
    try:
        ...
        if days not in ad_plans:
            return bot.reply_to(msg, "❌ Plan not found.")
        del ad_plans[days]
        save_ad_plans()
        bot.reply_to(msg, f"✔ Removed ad plan: {days} days")
    except Exception as e:
        bot.reply_to(msg, f"❌ Error: {e}")
...
def cmd_addpoints(msg):
    ...
    try:
        ...
        save_json(USERS_FILE, users)
        bot.reply_to(msg, f"✅ Added {amount} points to {target_id}")
        try:
            bot.send_message(int(target_id), f"💰 You received {amount} points from admin.")
        except Exception:
            pass
    except Exception as e:
        bot.reply_to(msg, f"❌ Error: {e}")
